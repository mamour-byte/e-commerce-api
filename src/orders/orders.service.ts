...
export class OrdersService {
  ...
  async create(
    ...
    // Le panier guest est identifié via le header X-Session-Id (même principe que /cart).
    // On le propage dans le DTO pour les visiteurs afin de valider l'appartenance du panier.
    if (!userId && headerSessionId && !dto.sessionId) {
      dto.sessionId = headerSessionId;
    }
    ...
    const fulfillmentType =
      dto.fulfillmentType || dto.deliveryMethod || FulfillmentType.PICKUP;
    ...
    if (fulfillmentType === FulfillmentType.DELIVERY) {
      if (neighborhoodId) {
        ...
        if (
          ...
          !neighborhood.deliveryZone.isActive
        ) {
        ...
        if (!zone) {
          throw new NotFoundException(
            'Zone de livraison introuvable ou inactive.',
          );
        }
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      ...
      // B. Débiter le stock à la création de la commande et tracer le mouvement
      for (const item of resolvedItems) {
        if (item.trackInventory) {
          if (item.variantId) {
            ...
            await this.syncParentStock(tx, item.productId);
          } else {
          ...
          await tx.inventoryMovement.create({
            data: {
              ...
              reason: `Sortie de stock pour commande ${createdOrder.orderNumber}`,
              productId: item.productId,
              variantId: item.variantId,
  ...
  async cancelMyOrder(userId: string, orderId: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      // Réintégrer le stock (annulation d'une commande dont le stock a été débité)
      for (const item of order.items) {
        if (!(await this.tracksInventory(tx, item.variantId, item.productId))) {
          continue;
        }
  ...
  async updateStatus(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Le stock est débité à la création de la commande (sortie), donc toute
      // transition vers CANCELLED/REFUNDED réintègre le stock (retour).
      // Une confirmation (PENDING -> CONFIRMED/IN_DELIVERY) ne touche plus au stock.
      if (
    ...
    switch (newStatus) {
      ...
      case OrderStatus.IN_DELIVERY:
        await this.notificationsService.notifyOrderInDelivery(
          order.id,
          order.orderNumber,
          order.userId || undefined,
        );
  ...
  async updatePaymentStatus(id: string, dto: UpdatePaymentStatusDto) {
    ...
    if (
      ...
      this.mailService
        ...
        .catch((err) =>
          this.logger.error(
            `Échec envoi email confirmation paiement commande ${updatedOrder.orderNumber}`,
            err,
          ),
        );
    }
  ...
  private async resolveOrderItems(userId: string | null, dto: CreateOrderDto) {
    ...
    if (dto.cartId || (!dto.items?.length && (userId || dto.sessionId))) {
      ...
      if (!cart || !cart.items.length) {
        throw new BadRequestException(
          'Le panier spécifié est vide ou introuvable.',
        );
      }

      // Contrôle d'appartenance du panier :
      ...
      throw new BadRequestException(
        'Veuillez fournir un panier actif ou des articles à commander.',
      );
    }
    ...
    for (const item of itemsToProcess) {
      ...
      if (!product) {
        throw new NotFoundException(
          `Le produit avec l'ID ${item.productId} est introuvable ou inactif.`,
        );
      }

      let variant: Awaited<
        ReturnType<typeof this.prisma.productVariant.findFirst>
      > = null;
      if (item.variantId) {
        ...
        if (!variant) {
          throw new NotFoundException(
            `La variante ${item.variantId} pour le produit ${product.name} est introuvable ou inactive.`,
          );
        }
      }

      // Vérification de la disponibilité du stock (uniquement si le stock est suivi)
      ...
      resolvedItems.push({
        ...
        quantity: item.quantity,
        unitPrice,
        trackInventory: variant ? variant.trackInventory : product.trackInventory,
        productSnapshot: {
          productId: product.id,
          name: product.name,
          variantId: variant?.id || null,
  ...
  // ses variantes. À appeler après toute mutation de stock d'une variante afin
  // que `product.quantity` reflète toujours la valeur réelle vendable du
  // produit (les produits à variantes ne touchent jamais directement la
  // quantité du parent).
  ...
  private async validateAndCalculateCoupon(
    ...
    if (coupon.startsAt && coupon.startsAt > now) {
      throw new BadRequestException(
        `Le code promo "${code}" n'est pas encore actif.`,
      );
    }
    ...
    if (coupon.usageLimit && coupon.usageCount >= coupon.usageLimit) {
      throw new BadRequestException(
        `Le code promo "${code}" a atteint sa limite d'utilisation.`,
      );
    }
