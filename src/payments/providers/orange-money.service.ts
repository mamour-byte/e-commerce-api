...
export class OrangeMoneyService {
	...
	constructor(private readonly config: ConfigService) {
		...
		this.baseUrl = this.config.get<string>('ORANGE_MONEY_API_URL') || 'https://api.orange.com';
	...
	async initiateWebPayment(params: {
		...
		const payload = {
			...
			notif_url: `${this.backendUrl}/payments/webhooks/orange-money`,
			reference: params.orderNumber,
		};

		try {
			const response = await fetch(`${this.baseUrl}/orange-money-webpayment/dev/v1/webpayment`, {
				method: 'POST',
				headers: {
					Authorization: this.authHeader.startsWith('Basic') || this.authHeader.startsWith('Bearer')
						? this.authHeader
			...
			this.logger.warn(`Connexion à l'API Orange Money en mode simulation ou erreur réseau.`);
			return this.getSimulatedResponse(params);
		}
	}
