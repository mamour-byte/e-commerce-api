...
export class WaveService {
	...
	constructor(private readonly config: ConfigService) {
		...
		this.baseUrl = this.config.get<string>('WAVE_API_URL') || 'https://api.wave.com/v1';
	...
	async createCheckoutSession(params: {
		...
		try {
			...
			this.logger.warn(`Connexion à l'API Wave en mode simulation ou erreur réseau.`);
			return this.getSimulatedSession(params);
		}
	}

	verifyWebhookSignature(signature: string, rawBody: string): boolean {
		if (!this.webhookSecret || this.webhookSecret.includes('sample')) {
			return true; // Accepter en mode dev si la clé n'est pas encore renseignée
		}
		try {
			...
			this.logger.error('Erreur lors de la vérification de la signature Wave Webhook', error);
			return false;
		}
	}
